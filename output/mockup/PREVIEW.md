# FINEWAVE 홈페이지 리뉴얼 — 시안 v2 미리보기 (DRAFT)

> **v2 변경점**: 클라이언트 요청 PPTX의 실사이트 캡처 5장을 기반으로
> **실제 장비 사진 · 실제 카피 · 실제 버튼 톤 · 실제 메뉴 구성(INTRO/2.45GHz/FINECOOL™/FRCCS/FAQ)** 반영.
> 빔 깜빡임도 실사이트 핸드피스 이미지 기반 off/on 프레임으로 교체.
>
> 움직이는 화면(빔 깜빡임·배경 드리프트·서서히 등장·TOP 페이드인)은 `index.html`을 브라우저에서 열어 확인 — 하단 "실물로 보는 법".
...
### 1) INTRO 히어로 — 실장비 + 실카피 "Go Fine. Get Defined."
